"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.University = void 0;
const graphql_1 = require("@nestjs/graphql");
const sequelize_typescript_1 = require("sequelize-typescript");
const user_entity_1 = require("../../user/entities/user.entity");
let University = class University extends sequelize_typescript_1.Model {
};
__decorate([
    (0, graphql_1.Field)(() => graphql_1.ID, { description: 'Identificador de la universidad.' }),
    sequelize_typescript_1.PrimaryKey,
    sequelize_typescript_1.Column,
    __metadata("design:type", String)
], University.prototype, "id", void 0);
__decorate([
    (0, graphql_1.Field)({ description: 'Nombre de la universidad.' }),
    sequelize_typescript_1.Column,
    __metadata("design:type", String)
], University.prototype, "name", void 0);
__decorate([
    (0, graphql_1.Field)({ nullable: true, description: 'Descripcion de la universidad.' }),
    sequelize_typescript_1.Column,
    __metadata("design:type", String)
], University.prototype, "description", void 0);
__decorate([
    (0, graphql_1.Field)({ description: 'Estado de la universidad.' }),
    sequelize_typescript_1.Column,
    __metadata("design:type", Boolean)
], University.prototype, "status", void 0);
__decorate([
    (0, graphql_1.Field)({ description: 'Fecha de creacion de la universidad.' }),
    sequelize_typescript_1.Column,
    __metadata("design:type", Date)
], University.prototype, "createdAt", void 0);
__decorate([
    (0, graphql_1.Field)({ description: 'Fecha de edicion de la universidad.' }),
    sequelize_typescript_1.Column,
    __metadata("design:type", Date)
], University.prototype, "updatedAt", void 0);
__decorate([
    (0, sequelize_typescript_1.HasMany)(() => user_entity_1.User),
    __metadata("design:type", Array)
], University.prototype, "users", void 0);
University = __decorate([
    (0, graphql_1.ObjectType)(),
    sequelize_typescript_1.Table
], University);
exports.University = University;
//# sourceMappingURL=university.entity.js.map